...
skill: boardroom
bucket: thinking
evaluated_skill_hash: 17c8d8edb3f4d13a5ae8992cce2b9adba9229e9b
...
|---|---|---|
| Predictability | weak | L88 — `ops_dominant` is keyed on fuzzy keyword-presence plus a "dominant frame" judgment with no tie-break; the same plan can route to a different voice-set across runs, the one place same-process determinism breaks. |
...
| Completion criteria | weak | L112 — "Output 3-5 critiques in voice's posture" gates on a count, not a checkable done-condition; an agent can hit 3 shallow critiques and call the voice complete. Stage 0 (L77) and the gate loop (L122) are sharp; this step invites premature completion. |
