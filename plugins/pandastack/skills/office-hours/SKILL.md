...
If the chosen approach routes to `/sprint` or `/team-orchestrate` (there is build work to do, not a pure decision), ALSO write an executable plan to `docs/plans/{YYYY-MM-DD}-{slug}.md`. Skip this stage for pure-decision briefs with no execution.
...
Then add ONE pointer line to the brief under `## Chosen approach`: `Executable plan: docs/plans/{date}-{slug}.md`. Print both paths.
